...
<form action="/webgame/index.php/admin/System/group_rule_add" method="post" enctype="multipart/form-data">
        ...
        <!-- checked="checked"  -->
        <?php if(empty($Auth_rule)): ?><h1>没有数据</h1><?php endif; ?>
		<?php if(is_array($Auth_rule)): $i = 0; $__LIST__ = $Auth_rule;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="list-group-item">
		<input type="checkbox" name="rule_ids[]" value="<?php echo ($vo["id"]); ?>" <?php if(in_array($vo['id'],$checked)){ echo 'checked="checked"'; } ?> >
			<label for="admin_group_menu_item_72"><span class="text-warning icon-rainbow"></span> <?php echo ($vo["title"]); ?></label>
		</li> 
...
<?php if(count($vo['pid']) >0){ ?>
	...
	<?php if(is_array($vo['pid'])): $i = 0; $__LIST__ = $vo['pid'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($i % 2 );++$i;?><li class="list-group-item">
		<input type="checkbox" name="rule_ids[]" value="<?php echo ($sub["id"]); ?>" <?php if(in_array($sub['id'],$checked)){ echo 'checked="checked"'; } ?>  >
			<label for="admin_group_menu_item_74"><span class="text-warning glyphicon glyphicon-file"></span> <?php echo ($sub["title"]); ?></label>
		</li><?php endforeach; endif; else: echo "" ;endif; ?>
