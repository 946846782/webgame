...
class ComhelperController extends Controller {
    ...
    protected function saveData($data){
        session('session_account',$data['mg_account']);
        session('session_memberid',$data['mg_id']);
        session('session_username',$data['mg_name']);
        session('mg_data',$data);
        session('expire',3600);
        cookie('cookie_account',$data['mg_account'],3600); // 指定cookie保存时间
        cookie('cookie_username',$data['mg_name'],3600); // 指定cookie保存时间
        $web = $this->get_client_browser();

        //记录登录信息
        $logData['mgid'] = $data['mg_id'];
        ...
        $LoginTable = M('Managerloginlog');
        $status = $LoginTable->data($logData)->add();
        //扩展 memcache 保存数据
    }
